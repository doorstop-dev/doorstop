...
UID, text, ref and links of the item, as well as the extended
reviewed attributes.
...
You can add the key-value pair of the extended reviewed attributes in .doorstop.yml file
which guides the generation of the item. The content of the extended reviewed attributes
will be presented in the newly added item.


#### Example: extended reviewed attributes
Modify the .doorstop.yml in the `reqs` folder to
...
Using the command `doorstop` which is to validate items, will print warnings like

```
WARNING: REQ001: missing key "type" for the mandatory attributes
WARNING: REQ001: missing key "verification-method" for the mandatory attributes
...
```

After the modification of the .doorstop.yml file, the item generated by the command
`doorstop add REQ` will generate an item which contains two attributes i.e., type
...
User can modify the contents of both attributes type and verification-method. The changes
of the contents will be included in the fingerprint, too. After using the command
`doorstop`, the unreviewed changes of REQ020.yml are detected as

```
WARNING: REQ: REQ020: unreviewed changes
```

Using `doorstop review REQ` can mark REQ020.yml as reviewed.
