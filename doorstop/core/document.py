...
class Document(BaseValidatable, BaseFileObject):  # pylint: disable=R0902
    ...
    def _get_issues_level(items):
        ...
        for item in items[1:]:
            # komische art des vergleiches da viel zu viele zuweisungen die vermieden hätten werden können,
            # allerdings wird code so ergänzt das möglichst wenig vom orginal verändert wird (Siehe add 09.01.2020)
            puid = prev.uid
            plev = prev.level
            nuid = item.uid
            nlev = item.level
