...
class TestTreeStrings(unittest.TestCase):
    ...
    def test_from_list(self):
        ...
        a = MockDocumentSkip(path=EMPTY, should_auto_save=True)
        a.prefix = 'A'
        b = MockDocumentSkip(path=EMPTY, should_auto_save=True)
        b.prefix = 'B'
        b.parent = 'A'
        c = MockDocumentSkip(path=EMPTY, should_auto_save=True)
        c.prefix = 'C'
        c.parent = 'B'
        docs = [a, b, c]
...
class TestTree(unittest.TestCase):
    ...
    def setUp(self):
        document = Document(path=SYS, should_auto_save=True)
        self.tree = Tree(document)
        document.tree = self.tree
        document = Document(path=FILES, should_auto_save=True)
    ...
    def test_palce_empty(self):
        ...
        doc = MockDocumentSkip.new(tree=tree,
                                   path=os.path.join(EMPTY, 'temp'), root=EMPTY, prefix='TEMP', should_auto_save=True)
    ...
    def test_new_document(self):
        ...
        self.tree.create_document(should_auto_save=True, path=EMPTY, value='_TEST', parent='REQ')

    def test_new_document_unknown_parent(self):
        """Verify an exception is raised for an unknown parent."""
        temp = tempfile.mkdtemp()
        self.assertRaises(DoorstopError, self.tree.create_document, should_auto_save=True, path=temp, value='_TEST', parent='UNKNOWN')
