...
def _lines_text(obj, indent=8, width=79, **_):
    ...
    for item in iter_items(obj):
        ...
        if item.heading:
            ...
            # Level and UID
            yield "{l:<{s}}{i}".format(l=level, s=indent, i=item.uid)
...
def _lines_markdown(obj, linkify=False):
    ...
    for item in iter_items(obj):
        ...
        if item.heading:
            ...
            # Level and UID
            standard = "{h} {l} {i}".format(h=heading, l=level, i=item.uid)
            attr_list = _format_md_attr_list(item, linkify)
            yield standard + attr_list
...
def _format_md_attr_list(item, linkify):
    ...
    return " {{: #{i} }}".format(i=item.uid) if linkify else ''
...
def _format_md_item_link(item, linkify=True):
    ...
    if linkify and is_item(item):
        return "[{i}]({p}.html#{i})".format(i=item.uid, p=item.document.prefix)
    else:
...
def _format_html_item_link(item, linkify=True):
    ...
    if linkify and is_item(item):
        link = '<a href="{p}.html#{i}">{i}</a>'.format(i=item.uid,
